...
Press any key to begin''')
    ...
    stim_circle = Circle(win, radius=20, edges=128, units='pix',
...
# 新增先生成 50 个 up + 50 个 down 的任务序列，然后随机打乱，再按顺序读取
    # ===== 生成任务序列（50 up + 50 down）并打乱 =====
    tasks = ['up'] * (trial_num // 2) + ['down'] * (trial_num // 2)
    random.shuffle(tasks)

    for i in range(trial_num):

        outlet.push_sample(markers['trial_start'])
